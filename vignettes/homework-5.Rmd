...
t1 <- table(as.vector(preds.in1), y_train[s])
sum(diag(t1)) / sum(t1) 
...
sum(diag(t3)) / sum(t3) ## prediction accuracy on training dataset, 0.978

# out-of-sample test, accuracy on testing dataset
preds.test2 <- predict(fit$glmnet.fit, x_test, s = fit$lambda.min, type = "class")
t4 <- table(as.vector(preds.test2), y_test)
sum(diag(t4)) / sum(t4) ## prediction accuracy on testing dataset, 0.8515
```
...
t4 <- table(as.vector(preds.train3), y_train[s])
sum(diag(t5)) / sum(t5) ## prediction accuracy on training dataset, 0.982

# out-of-sample test, accuracy on testing dataset
preds.test3 <- predict(fit$glmnet.fit, x_test, s = fit$lambda.min, type = "class")
t5 <- table(as.vector(preds.test3), y_test)
...
Since I set Epoch = 10, R need to train 60,000 samples in 10 times. Becasue of the limitation in my CPU, it crashed several times on my computer. In case of such problems, I also attached the result photo here.
```{r, eval=FALSE}
...
#convert the value to a binary data
...
From the output above, the out-of-sample prediction accuracy is greatly increased in the CNN method. In this way, the potential overfitting problem is solved. It represents that CNN plays a better role in identifying handwriting characters than LASSO.

![Prediction Accuracy on Training and Testing Dataset](/Users/tianyuanzhang/Desktop/accuracy.png)
...
```{r,eval=FALSE}
#since the kernel size will influence the final accuracy, in order to find a suitable kernel size, I tried
